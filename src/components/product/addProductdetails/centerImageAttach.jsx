  ...
  return (
    <div className='mt-40'>
      <div>
        ...
        <h5 className='text-xs mt-1 text-gray-600'>Attach jpg.png with size less than 4mb.</h5>
        <div className='flex flex-wrap w-2/2'>
          <label htmlFor="fileInput" className='text-sm text-blue-400 border border-dashed border-blue-400 w-40 h-[110px] px-7 py-8 rounded-lg mt-4 mr-4 bg-blue-50 cursor-pointer'>
            <input
              type="file"
              id="fileInput"
              className="hidden"
          ...
          {Array.from({ length: Math.max(0, 3 - selectedImages.length) }).map((_, index) => (
            <div key={index} className={`flex text-sm text-blue-400 w-40 h-[110px] rounded-lg px-2 py-2 mr-4 mt-4 bg-blue-100`}>
              <label htmlFor={`fileInput-${index}`} className="w-full h-full cursor-pointer">
                <input
                  type="file"
                  id={`fileInput-${index}`}
                  className="hidden"
...
export default CenterImageAttach;
